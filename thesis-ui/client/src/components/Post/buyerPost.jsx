...
class BuyerPost extends Component {
  ...
  getBartering = async () => {
    try {
      ...
      const { data } = await axios.get(`${REST_SERVER_URL}/api/offers/getSingleOffer/${buyerUsername}/${postId}`);
      console.log(
  ...
  getFollowing = async () => {
    try {
      ...
      const { data } = await axios.get(`${REST_SERVER_URL}/api/followings/single/${followerId}/${userId}`);
  ...
  getWatching = async () => {
    try {
      ...
      const postId = this.props.current_post.id;
      const { data } = await axios.get(`${REST_SERVER_URL}/api/watchers/${userId}/${postId}`);
  ...
  toggleFollowList = async () => {
    ...
    if (localStorage.id) {
      if (this.state.currentlyFollowing === true) {
        await axios.delete(`${REST_SERVER_URL}/api/followings/${followerId}/${userId}`);
        this.setState({
          currentlyFollowing: false,
        });
        console.log('you are no longer following this user!');
      } else {
        await axios.post(`${REST_SERVER_URL}/api/followings/${followerId}/${userId}`);
        this.setState({
          currentlyFollowing: true,
        });
  ...
  async cancelOffer() {
    ...
    try {
      ...
      const postId = this.props.current_post.id;
      await axios.delete(`${REST_SERVER_URL}/api/offers/deleteOffer/${userName}/${postId}`);
      this.setState({
        bartering: false,
      });
