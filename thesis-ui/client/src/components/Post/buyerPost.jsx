...
class BuyerPost extends Component {
  ...
  getBartering = async () => {
    try {
      ...
      const { data } = await axios.get(
        `http://localhost:3396/api/offers/getSingleOffer/${buyerUsername}/${postId}`
      );
      console.log(
  ...
  getFollowing = async () => {
    try {
      ...
      const { data } = await axios.get(
        `http://localhost:3396/api/followings/single/${followerId}/${userId}`
      );
  ...
  async cancelOffer() {
    ...
    try {
      ...
      const postId = this.props.current_post.id;
      await axios.delete(
        `http://localhost:3396/api/offers/deleteOffer/${userName}/${postId}`
      );
      this.setState({
        bartering: false
      });
